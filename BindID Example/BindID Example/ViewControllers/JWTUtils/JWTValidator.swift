...
class JWTValidator {
    ...
    public func validate(_ idToken: String, callback: @escaping (Bool, JWTValidatorError?) -> Void) {
        ...
        fetchBindIDPublicKey { certificateBase64 in
            ...
            guard let iss = decodedPayload["iss"] as? String, // get issuer
                  ...
                  issuer.host == Environment.hostName.deletingPrefix("https://"), // compare issuer with BindID host
    ...
    private func fetchBindIDPublicKey(callback: @escaping (String?) -> Void) {
        let publicKeyEndPoint = "\(Environment.hostName)/jwks"
