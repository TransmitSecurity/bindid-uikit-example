...
class JWTParser {
    ...
    public func parseIDTokenData(_ tokenData: [String: Any]) -> [String: String] {
        ...
        passport["04. User Registered On"] = networkInfo?[IDTokenKeys.userRegistrationTime.rawValue] as? String
        passport["05. User First Seen"] = formatTimestamp(bindIDInfo?[IDTokenKeys.firstLogin.rawValue] as? Double)
        passport["06. User First Confirmed"] = formatTimestamp(bindIDInfo?[IDTokenKeys.firstConfirmedLogin.rawValue] as? Double)
        passport["07. User Last Seen"] = formatTimestamp(bindIDInfo?[IDTokenKeys.cappLastLogin.rawValue] as? Double)
        passport["08. User Last Seen by Network"] = networkInfo?[IDTokenKeys.userLastSeen.rawValue] as? String
        passport["09. Total Providers that Confirmed User"] = "\(networkInfo?[IDTokenKeys.confirmedCappCount.rawValue] as? Int ?? 0)"
        passport["10. Authenticating Device Registered"] = formatTimestamp(bindIDInfo?[IDTokenKeys.lastLoginFromAuthenticatedDevice.rawValue] as? Double)
        
        let acr = tokenData["acr"] as? [String] ?? []
        passport["11. Authenticating Device Confirmed"] = acr.contains(IDTokenKeys.bindIDAppBoundCred.rawValue) ? "Yes" : "No"
        
        passport["12. Authenticating Device Last Seen"] = formatTimestamp(bindIDInfo?[IDTokenKeys.lastLoginFromAuthenticatedDevice.rawValue] as? Double)
        
        passport["13. Authenticating Device Last Seen by Network"] = networkInfo?[IDTokenKeys.authenticatedDeviceLastSeen.rawValue] as? String ?? kNoStringString
        passport["14. Total Known Devices"] = "\(networkInfo?[IDTokenKeys.deviceCount.rawValue] as? Int ?? 0)"
